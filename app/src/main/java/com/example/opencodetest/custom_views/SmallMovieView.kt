...
class SmallMovieView(context: Context, val movie: Movie, onClose: () -> Unit) :
        ...
        when (metadata) {
            is ResOk -> {
                ...
                metadata.value.genres?.let {
                    popupWindow.contentView.popUpMovieGenre.text = arrayToCommaString(it)
                }
                metadata.value.directors?.let {
                    popupWindow.contentView.popupMovieDirector.text = arrayToCommaString(it)
                }
                metadata.value.actors?.let {
                    popupWindow.contentView.popupMovieActors.text = arrayToCommaString(it)
                }
                ...
                metadata.value.duration?.let {
                    smallMovieDuration.text = getDurationFromMinutes(it)
                }
                metadata.value.score?.let { score -> smallMovieScore10.text = score.toString() }
            }
            is ResError -> when (metadata.value) {
                is ConnectionError -> popupWindow.contentView.popupMovieError.text =
                    "Оффлайн версия"
            }
        }
    }
